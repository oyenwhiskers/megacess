...
class AnalyticsView extends StatelessWidget {
  ...
  future: AttendanceService(SecureStorageService()).fetchCheckerAnalytics(),
        builder: (context, snapshot) {
          ...
          final pendingRatio = totalTasks > 0 ? (pending / totalTasks * 100) : 0.0;
          final completedRatio = totalTasks > 0 ? (completed / totalTasks * 100) : 0.0;
          return FutureBuilder<PendingTaskListResponse>(
            future: AttendanceService(SecureStorageService()).fetchPendingTasks(),
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            Text(pending.toString(), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.amber)),
                          ],
                        ),
                      ),
                              ...
                              child: Column(
                                children: [
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: [
                                      const Text('Absent', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                                      const SizedBox(width: 6),
                                      const Icon(Icons.remove_circle, color: Colors.red, size: 20),
                                    ],
                                  ),
                                  const SizedBox(height: 6),
                                  Text(absent.toString(), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.red)),
                                ],
                              ),
                            ),
                          ),
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  const SizedBox(height: 6),
                                  Text(completed.toString(), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.green)),
                                ],
                              ),
                            ),
                          ),
                            ...
                            PieChartData(
                              sections: [
                                PieChartSectionData(
                                  ...
                                  titleStyle: const TextStyle(fontWeight: FontWeight.bold, fontSize: 22, color: Color(0xFF222B45)),
                                  radius: 110,
                                ),
                                PieChartSectionData(
                                  ...
                                  titleStyle: const TextStyle(fontWeight: FontWeight.bold, fontSize: 22, color: Colors.white),
                                  radius: 110,
                                ),
                              ],
                              sectionsSpace: 0,
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        const Text('Tasks to be verified:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        const SizedBox(height: 12),
                        if (pendingSnapshot.connectionState == ConnectionState.waiting)
                          const Center(child: CircularProgressIndicator()),
                        if (pendingSnapshot.hasError)
                          const Center(child: Text('Failed to load pending tasks.')),
                        if (pendingSnapshot.hasData)
                          ...pendingSnapshot.data!.tasks.map((task) => Padding(
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: Padding(
                                          padding: const EdgeInsets.only(top: 14, left: 0, right: 8, bottom: 8),
                                          child: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text(
                        ...
                        if (pendingSnapshot.hasData && pendingSnapshot.data!.tasks.isEmpty)
