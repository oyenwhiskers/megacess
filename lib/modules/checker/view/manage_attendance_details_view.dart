...
class ManageAttendanceDetailsView extends StatefulWidget {
  ...
  const ManageAttendanceDetailsView({Key? key, required this.dateAttendanceId, required this.dateLabel}) : super(key: key);

  @override
  State<ManageAttendanceDetailsView> createState() => _ManageAttendanceDetailsViewState();
}

class _ManageAttendanceDetailsViewState extends State<ManageAttendanceDetailsView> {
      ...
      body: Column(
        children: [
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text('Management',
                          style: TextStyle(
                            color: _selectedTab == 0 ? const Color(0xFF43C463) : Colors.grey,
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                          ),
                        ),
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Center(
                        child: Text('Staff',
                          style: TextStyle(
                            color: _selectedTab == 1 ? const Color(0xFF43C463) : Colors.grey,
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                          ),
                        ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                Text('Date: ${widget.dateLabel}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              ],
            ),
          ),
          Expanded(
                ...
                ? FutureBuilder<UserAttendanceListResponse>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return Center(child: Text('Failed to load user attendance.'));
                      } else if (!snapshot.hasData || snapshot.data!.data.isEmpty) {
                        return const Center(child: Text('No user attendance records found.'));
                      }
                      final attendanceList = snapshot.data!;
                      // Filter by search
                      final filtered = _search.isEmpty
                          ? attendanceList.data
                          : attendanceList.data.where((item) => item.userName.toLowerCase().contains(_search.toLowerCase())).toList();
                      if (filtered.isEmpty) {
                        return const Center(child: Text('No user attendance records found.'));
                      }
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            ...
                            child: ListTile(
                              ...
                              title: Text('Name: ${item.userName}', style: const TextStyle(fontWeight: FontWeight.bold)),
                              subtitle: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Status: ${item.status}'),
                                  Text('Checked by: ${item.checkedinBy ?? '-'}'),
                                ],
                              ),
                ...
                : FutureBuilder<StaffAttendanceListResponse>(
                    ...
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return Center(child: Text('Failed to load staff attendance.'));
                      } else if (!snapshot.hasData || snapshot.data!.data.isEmpty) {
                        return const Center(child: Text('No staff attendance records found.'));
                      }
                      final staffList = snapshot.data!;
                      // Filter by search
                      final filtered = _search.isEmpty
                          ? staffList.data
                          : staffList.data.where((item) => item.staffName.toLowerCase().contains(_search.toLowerCase())).toList();
                      if (filtered.isEmpty) {
                        return const Center(child: Text('No staff attendance records found.'));
                      }
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Container(
                            ...
                            child: ListTile(
                              ...
                              title: Text('Name: ${item.staffName}', style: const TextStyle(fontWeight: FontWeight.bold)),
                              subtitle: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('Status: ${item.status}'),
